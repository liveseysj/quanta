...
class LowDrawdownStrategy:
    ...
    def generate_signal(self):
        """
        Strategy targeting DD < 17% with Calmar > 2.
        More aggressive than ultra-defensive version.
        """
        ...
        # Higher vol target now that we allow 17% DD
        BASE_VOL = 0.18
        ...
        for i in range(252, len(df)):
            ...
            low_vol = vol_pct < 40
            very_low_vol = vol_pct < 30

            is_perfect = all_aligned and strong_momentum and low_vol
            is_excellent = all_aligned and mom_20 > 0 and mom_60 > 0 and vol_pct < 50
            is_good = above_200 and above_50 and mom_60 > 0

            # === MEAN REVERSION SETUP ===
            oversold_bounce = above_50 and rsi < 30 and mom_5 > 0
            ibs_bounce = above_50 and ibs < 0.2 and rsi < 40
            deep_oversold = above_200 and rsi < 25 and dist_20 < -0.04

            # === BASE SIGNAL (aggressive for Calmar > 2) ===
            if is_perfect:
                if very_low_vol and mom_20 > 0.03:
                    base = 2.0
                elif very_low_vol and mom_20 > 0.02:
                    base = 1.8
                elif very_low_vol:
                    base = 1.5
                else:
            ...
            # === MEAN REVERSION BOOST (higher win rate setups) ===
            if deep_oversold:
                base += 0.7
                ...
                base += 0.45
            elif above_50 and rsi < 32:
                base += 0.25
            elif above_50 and rsi < 38:
                base += 0.12

            # === MOMENTUM BOOST ===
            if all_aligned and mom_20 > 0.05:
                base += 0.4
            elif all_aligned and mom_20 > 0.03:
                base += 0.3
            elif all_aligned and mom_20 > 0.02:
                base += 0.2

            # === LOW VOL REGIME BOOST ===
            if very_low_vol and all_aligned:
                base *= 1.15

            # === VOL TARGETING ===
            vol_mult = BASE_VOL / curr_vol
            vol_mult = min(vol_mult, 1.8)

            # Vol regime scaling (balanced for returns vs DD)
            if vol_pct > 80:
                vol_mult *= 0.25
            elif vol_pct > 70:
                vol_mult *= 0.40
            ...
            # === RECENT WEAKNESS ===
            if i > 5:
                five_day = df['Returns'].iloc[i-5:i].sum()
                if five_day < -0.08:
                    base *= 0.2
                elif five_day < -0.05:
                    base *= 0.4
    ...
    def print_results(self, results):
        ...
        for period, res in results.items():
            dd_ok = abs(res['max_drawdown']) < 0.17
            ...
            print(f"  Max Drawdown:    {res['max_drawdown']:.1%}" +
                  (" [OK < 17%]" if dd_ok else " [FAIL]"))
            print(f"  Calmar Ratio:    {res['calmar']:.3f}" +
                  (" [OK > 2.0]" if calmar_ok else " [FAIL]"))

        print("\n" + "=" * 60)
        blind = results['blind']
        if abs(blind['max_drawdown']) < 0.17 and blind['calmar'] > 2.0:
            print("SUCCESS: Blind period meets DD < 17% and Calmar > 2.0!")
        else:
