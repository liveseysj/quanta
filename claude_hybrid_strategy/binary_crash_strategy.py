"""
Binary Crash Strategy - Max Alpha During Bulls, Zero During Bears
=================================================================

Key principle: Market timing is binary.
- Either we're in a bull market (trade aggressively)
- Or we're in a bear market (stay in cash)

The 2000-2002 crash was -83%. The 2008 crash was -56%.
To limit DD to 17%, we MUST exit before these crashes develop.

Crash exit signals:
1. Price breaks below 200 SMA with momentum confirmation
2. Death cross (50 below 200 SMA)
3. 60-day drawdown > 15%

Recovery entry signals:
1. Price above 200 SMA for 10+ days
...
class BinaryCrashStrategy:
    """Binary market participation strategy."""
    ...
    def _calculate_features(self):
        ...
        df['Days_Above_200'] = df['Above_200'].rolling(20).sum()

    def generate_signal(self):
        """Generate trading signal with binary crash protection."""
        print("Generating binary crash signals...")
        ...
        # Track market state
        in_bull = True  # Start assuming bull market
        days_in_cash = 0
        ...
        for i in range(252, n):
            ...
            days_above = df['Days_Above_200'].iloc[i]
            ...
            # 1. Break below 200 SMA with negative momentum
            if not above_200 and mom_60 < -0.05:
                crash_signal = True

            # 2. Death cross with downtrend
            if not golden_cross and not above_200 and mom_20 < 0:
                crash_signal = True

            # 3. 60-day drawdown > 12%
            if dd_60 < -0.12 and not above_200:
                crash_signal = True

            # 4. Severe distance from 200 SMA
            if dist_200 < -0.15:
                crash_signal = True

            # 5. Panic vol with downtrend
            if vol_pct > 80 and not above_200 and mom_20 < -0.05:
                crash_signal = True
            ...
            # =====================================================
            # RECOVERY DETECTION - Re-enter market
            # =====================================================
            if not in_bull:
                days_in_cash += 1
                ...
                # Need strong confirmation (at least 5 conditions)
                if recovery_score >= 5 and days_in_cash >= 20:
                    in_bull = True
            ...
            # =====================================================
            if not in_bull:
                # In cash - only small bounce trades
                if rsi < 15 and ibs < 0.1 and dd_60 < -0.20:
                    signal.iloc[i] = 0.3  # Small bounce
                else:
                    signal.iloc[i] = 0.0
                continue
            ...
            # Moderate trend
            if above_200 and above_50 and mom_60 > 0.05:
                sig += 0.15
    ...
    def optimize_for_dd(self, target_dd=-0.17):
        ...
        if worst_dd < target_dd:
            ...
            for scale in np.arange(1.05, 2.0, 0.05):
                ...
                if test_worst_dd >= target_dd:
                    ...
                    print(f"Scaled up {scale:.2f}x, worst DD: {test_worst_dd:.1%}")
                else:
                    break
    ...
    def print_results(self, results):
        """Print results."""
        print("\n" + "=" * 60)
        print("BINARY CRASH STRATEGY - RESULTS")
        print("=" * 60)
    ...
    def plot_results(self, signal, results):
        ...
        plt.tight_layout()
        plt.savefig('binary_crash_results.png', dpi=150, bbox_inches='tight')
        plt.close()

        print("\nSaved: binary_crash_results.png")
...
def main():
    ...
    for train_path, blind_path in data_paths:
        try:
            strategy = BinaryCrashStrategy(train_path, blind_path)
            strategy.load_data()
            break
        except:
            continue
